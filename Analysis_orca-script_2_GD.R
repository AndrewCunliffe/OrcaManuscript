...
### Analysis of moss cover effect on NDVI-biomass relationships ####
  ...
  # Mixed effects model to test the effect of moss_prop on the NDVI-biomass relationships #
    ...
    # From Gergana: Here I think you are looking for an mean_NDVI_121 interaction with moss_prop.
    # the coefficient of that interaction effect will tell you about how moss_prop influences
    # the phytomass/NDVI relationship
    
    # Since we have multiple records from the same plot (the different points for which you extracted the NDVI
    # (or in other cases, canopy height), we want to account for that pseudoreplication by having a plot random effect
    # for this the data also needs to be in a long format versus wide
    
    # I see mean_NDVI per plot here - do you have also the raw values from which the mean was calculated?
    # That's what you'd need for the model
    # I've written out my suggestion for the model structure before so once you have the values,
    # I think you can just run this model (and change the variable names if they are called something slightly different)
    
model2 <- lmer(phytomass ~ NDVI_121*moss_prop + (1|PlotID), data = dataset_long)

# Check the distribution of phytomass
hist(dataset$phytomass)
hist(log(dataset$phytomass))

# I think you might need to log the phytomass

# Random effects can only be categorical and you need to have more than one observation per whatever
# the random effect is
# so to have a plot random effect, we need multiple NDVI values not their overall mean

# without a hierarchical model, you can still look at the interactive effect of moss:
...
# I think this graph illustrates the interaction well and we can use that approach for the other types of 
# biomass and any other interactions we might want to test
...
# Figure 3. NEW including NDVI ----
  ...
  (biomass_CH_SfM <- ggplot(data = dataset,
      ...
      theme(legend.position = c(0.15, 0.9)) +
      geom_smooth(method="lm", formula= y ~ x-1, se=TRUE, size=0.5, na.rm = TRUE, colour = "black"))
  ...
  # Export figure
  png(filename="plots/Figure 3 - NEW Biomass Predictions_2.png", width=20, height=7, units="cm", res=400)
  plot(biomass_plots)
  dev.off()
